...
public class HomeServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("<title>“елефонный справочник</title>");
        out.println("</head>");
        out.println("<body>");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        //метод срабатывает только если в форме заполненно хот€ бы одно поле поиска
        response.setContentType("text/html;charset=UTF-8");
        response.setCharacterEncoding("UTF-8");
        request.setCharacterEncoding("UTF-8");//если передаютс€ русские символы
        PrintWriter out = response.getWriter();
          ...
          try{
              ...
              while (vIt.hasNext()){
                 ...
                 /* out.println("<div class=\"col-4 align-self-center text-right\">");
                  ...
                  out.println("</div>"); */                   
                  out.println("<div class=\"col-8 width-height-img align-self-center text-left text-white font-italic\"> " + itemU.getFirstName() + " " + itemU.getLastName() + " phone " + itemU.getPhone() + "</div>");
                  out.println("</div>");             
                  out.println("<div class=\"row separator_row\">");
                  out.println("<div class=\"col-12\"><hr/></div>");   
                  out.println("</div>");  
    ...
    public void displayDataPgSql(PrintWriter out, List<Users> users, boolean home){
       ...
       if (users.size()==0){
            //если вызываетс€ home == true и users.size()==0 значит просто нет данных в базе 
            //если вызываетс€ home == false и users.size()==0 значит пользователь ввел не правильные данные которых нет в базе 
            if(home == false){
